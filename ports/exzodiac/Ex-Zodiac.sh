...
# make godot fix the guid for us
# (export template won't run unless you give it a project file)
echo 'config_version=5' > /tmp/project.godot
$ESUDO $godot_dir/$godot_executable --path /tmp --headless -s $GAMEDIR/guid_hack.gd
rm -f /tmp/project.godot

# the environment variable just in case
if [ -f /tmp/gamecontrollerdb.txt ]; then
  export SDL_GAMECONTROLLERCONFIG="$(< /tmp/gamecontrollerdb.txt)"
fi
